...
export class TableBody<D> extends React.Component<TableBodyProps<D>, never> {
    render() {
        return <div style={{ display: 'grid', position: 'absolute', width: '100%', gridAutoRows: this.props.rowHeight, gridTemplateColumns: this.props.sizes.map((fraction) => `${fraction * 100}%`).join(' '), top: this.props.headHeight + this.props.start * this.props.rowHeight }}>
